...
def handler(event: dict, context) -> dict:
    ...
    if method == 'POST' and 'comments' in path:
        ...
        cur.execute(
            ...
            (body.get('stage_id'), user['name'], body.get('text', ''))
        )
        row = cur.fetchone()
        conn.commit()
        conn.close()
        return respond(201, {'id': row[0], 'stage_id': body.get('stage_id'), 'author': user['name'], 'text': body.get('text', ''), 'created_at': str(row[1])})

    # ── GET /stages?deal_id=X ─────────────────────────────────────────────────
    if method == 'GET' and 'stages' in path:
    ...
    # ── POST /stages ──────────────────────────────────────────────────────────
    if method == 'POST' and 'stages' in path:
        body = json.loads(event.get('body') or '{}')
        cur.execute(
            f"INSERT INTO {SCHEMA}.deal_stages (deal_id,title) VALUES (%s,%s) RETURNING id,created_at",
            (body.get('deal_id'), body.get('title', ''))
        )
        row = cur.fetchone()
        conn.commit()
        conn.close()
        return respond(201, {'id': row[0], 'deal_id': body.get('deal_id'), 'title': body.get('title', ''),
                              'status': 'pending', 'taken_by': None, 'taken_at': None, 'completed_at': None, 'created_at': str(row[1])})

    # ── PUT /stages — взять в работу / завершить / возобновить ───────────────
    if method == 'PUT' and 'stages' in path:
        body = json.loads(event.get('body') or '{}')
        stage_id = body.get('id')
        action = body.get('action')  # take | complete | reopen
    ...
    # ── GET /?client_id=X — список сделок клиента ─────────────────────────────
    if method == 'GET':
    ...
    if method == 'POST':
        ...
        # Создать начальные этапы если переданы
